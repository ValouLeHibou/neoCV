...
function switchToFire() {
    ...
    document.getElementById("webcam").src = "./secretProject/fire";
...
function switchToPhoto() {
    ...
    document.getElementById("webcam").src = "./secretProject/photo";
...
function switchWebCam() {
    setTimeout(function () {
        ...
        if (webcam_src.search("./secretProject/fire") === -1 &&
            webcam_src.search("./secretProject/photo") === -1)
        {
...
function timeLoop() {
    ...
    setTimeout(function () {
        ...
        if (hour >= photoStartHour && hour < photoEndHour && webcam_src.search("./secretProject/photo") === -1) {
            switchToPhoto();
        }
        if (hour >= fireStartHour && hour < fireEndHour && webcam_src.search("./secretProject/fire") === -1) {
            switchToFire();
        }
        timeLoop()
